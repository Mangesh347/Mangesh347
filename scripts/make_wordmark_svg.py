...
# two letters (ML) leave enough grid columns each for the extrusion to read as 3D.
TEXT = os.environ.get("WORDMARK_TEXT", "ML")
...
def emit(frames, mode, out, dur, reveal):
    ...
    p.append(f'<text x="{canvas_w/2:.0f}" y="{TITLEBAR_H/2 + 4:.0f}" fill="{TITLE_TEXT}" '
             f'font-size="11.5" text-anchor="middle">mangesh@github: ~$ ./wordmark.sh --3d</text>')
